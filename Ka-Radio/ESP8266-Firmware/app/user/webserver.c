...
//	printf ("Heap size: %d\n",xPortGetFreeHeapSize( ));
	...
	{
		char *con = NULL;
		con = (char*) inmalloc( (gpart)*sizeof(char) );		// if malloc successful from first attempt, no need to reduce gpart
		while ( (con == NULL) && (gpart >= 32) ) {
			gpart /=2;
			con = (char*) inmalloc( (gpart)*sizeof(char) );
			printf("++++++++++ !!!!!!!! con = %x, gepart = %d Heap size: %d\n", con, gpart, xPortGetFreeHeapSize());
		}
//		do {
//			con = (char*)inmalloc((gpart)*sizeof(char));
//			gpart /=2;
