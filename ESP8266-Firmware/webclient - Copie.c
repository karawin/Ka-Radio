		...
		if((chunked != 0)&&((cchunk ==0)||(len >= cchunk-1)))  //if in chunked mode and chunk received or complete in data
		{
			...
			{	
				...
				else		// a part of end of chunk 	and beginnining of a new one
				{
					...
					while (lc < cchunk+8) {clen = recv(sockfd, inpdata+len, RECEIVE+8-len, 0); lc+=clen;len+=clen;} //security to be sure to receive the new length
				...
				printf("leni:%d, chunked:%d  cchunk:%d, lc:%d, metad:%d  str: %s\n",len,chunked,cchunk, lc,metad,inpdata+cchunk );
				...
				printf("leno:%d, chunked:%d  cchunk:%d, lc:%d, metad:%d  str: %s\n",len,chunked,cchunk, lc,metad,inpdata+cchunk );
					...
					if (cchunk >1)
						memcpy (inpdata+cchunk-2,inpdata+len-lc, lc); 
					else
						memcpy (inpdata,inpdata+len-lc, lc); 
					len -=  clen;
					...
					else
					{
						...
						cchunk = chunked;
						printf("Inlen, len:%d, lc:%d, pdata:%x, inpdata:%x, ccchunk:%d, str:%s\n",len,lc,pdata,inpdata,cchunk,inpdata+chunked);
					}	
